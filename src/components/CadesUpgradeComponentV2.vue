<script setup lang="ts">
  ...
  import { SignatureLevelForCades,  type ProxyGetSignatureListResult, type ProxyGetSignatureListResultItem, type ProxyUploadFileResultV2, type ProxyGetSignatureListResultV3, type ProxyGetSignatureListResultItemV3 } from "@/types/Types";
  ...
  const operationIdOfUpgradeCades = ref("");
  ...
  const signatureOptions = Object.keys(SignatureLevelForCades).filter((key) => isNaN(Number(key))).map((key) => {
    return {
      label: key,
  ...
  const selectedCadesSignatureLevel = ref(signatureOptions[0]);
  ...
  function onFileSelected(event: Event) {
    ...
    if (target?.files && target.files.length > 0) {
      ...
      operationIdOfUpgradeCades.value="";
  ...
  function GetSignatureListCades() {
    ...
    axios
      ...
      .catch((error) => {
        ...
        waitString.value = "Cades imza listesi alınamadı. " + HandleError(error);
      });
  }
  
  function DownloadFile() {
    axios
    .get(store.API_URL + "/Onaylarim/DownloadCoreV2?operationId=" + operationIdOfUpgradeCades.value, { responseType: "blob" })
      .then((e) => {
        if (e.data.error) {
  ...
  function UpgradeCades() {
  
    logs.value.push("Sizin sunucu katmanına UpgradeCades isteği gönderiliyor.");
    axios
      .get(store.API_URL + "/Onaylarim/UpgradeCadesV2?operationId=" + operationIdOfFileUpload.value + "&signatureLevel=" + selectedCadesSignatureLevel.value.value + "&signaturePath=" + signaturePath.value)
      .then((e) => {
        logs.value.push("Sizin sunucu katmanına UpgradeCades isteği gönderildi. Detaylar için console'a bakınız.");
        console.log("Sizin sunucu katmanına UpgradeCades isteği gönderildi.", e);
        if (e.data.error) {
          waitString.value = "Hata oluştu. " + e.data.error;
        } else {
          logs.value.push("e-İmzalar zenginleştirildi.");
          console.log("e.data", e.data);
          operationIdOfUpgradeCades.value = e.data as string;
          isSuccess.value = true;
          console.log("e", e.data);
        }
      })
      .catch((error) => {
        logs.value.push("Sizin sunucu katmanına UpgradeCades isteği gönderilemedi. Mesaj: " + HandleError(error) + " Detaylar için console'a bakınız.");
        console.log("Sizin sunucu katmanına UpgradeCades isteği gönderilemedi.", error);
      });
  }
  ...
  <template>
    <main class="space-y-4">
      <CardComponent title="Cades Upgrade V2">
        ...
        <template v-slot:content>
          <div class="flex items-end ">
                  ...
                  <div class="relative mt-0">
                    ...
                    <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                      <ListboxOptions
                        ...
                        <ListboxOption as="template" v-for="cadesSignatureLevel in signatureOptions" :key="cadesSignatureLevel.label" :value="cadesSignatureLevel" v-slot="{ active, selected }">
            ...
            <div>
              ...
              <button type="button" @click="UpgradeCades()" :disabled="!operationIdOfFileUpload"
