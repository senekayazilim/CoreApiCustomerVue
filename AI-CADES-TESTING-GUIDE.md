# 🤖 AI CAdES Test Rehberi - ONAYLARIM Prime API Demo

Bu dokümantasyon, **Cursor AI** veya benzeri AI asistanların browser automation araçlarını kullanarak CAdES e-imza testlerini çalıştırabilmesi için hazırlanmıştır.
...
```
@browser AI-CADES-TESTING-GUIDE.md dosyasındaki CAdES testlerini çalıştır
```
...
## ⬇️ Dosya İndirme Yöntemi (ÖNEMLİ!)

**⚠️ curl kullanmayın!** curl ile indirilen dosyalar bozuk olabilir.

**Doğru Yöntem:**
1. İmzalama tamamlandıktan sonra "e-İmzalı dosyayı indir" linkine **tıklayın**
2. Dosya Playwright artifacts klasörüne indirilir:
   ```
   C:\Users\<USER>\AppData\Local\Temp\playwright-artifacts-*\<GUID>
   ```
3. En son indirilen dosyayı bulun ve `public/docs/` altına kopyalayın:
   ```powershell
   # Son indirilen dosyayı bul ve kopyala
   $artifact = Get-ChildItem "$env:LOCALAPPDATA\Temp\playwright-artifacts-*\*" -File | 
       Sort-Object LastWriteTime -Descending | Select-Object -First 1
   Copy-Item $artifact.FullName "public\docs\<HEDEF_DOSYA_ADI>"
   ```
